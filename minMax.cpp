...
int checkWin(const vector<int>& board, int player) { 
    for (int i = 0; i < 3; i++) {
if ((board[i * 3] == player && board[i * 3 + 1] == player && board[i * 3 + 2] == player) || (board[i] == player && board[i + 3] == player && board[i + 6]
== player)) {
return 10;
}
}
...
int minimax(vector<int>& board, int depth, int maxDepth) { count++;
...
int currentScore = -minimax(board, depth + 1, maxDepth); board[move] = 2;
...
for (int move : moves_left(board)) { board[move] = Computer;
int score = minimax(board, 0, maxDepth) + rate(board, Computer,
...
} else if (computerResult == 10) { display();
