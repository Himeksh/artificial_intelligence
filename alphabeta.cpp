...
int rate(const vector<int> &board, int player, int turn)
{
    int score = 0;
...
int alphaBeta(vector<int> &board, int depth, int alpha, int beta, int maxDepth)
{
    count++;
    ...
    if (depth >= maxDepth)
        return rate(board, Computer, turn);
    ...
    for (int move : moves)
    {
        ...
        int currentScore = -alphaBeta(board, depth + 1, -beta, -alpha, maxDepth);
        board[move] = 2;
...
void computerTurn()
{
    ...
    for (int move : moves_left(board))
    {
        ...
        int score = alphaBeta(board, 0, -1000, 1000, maxDepth) + rate(board, Computer, turn);
        board[move] = 2;
...
int main()
{
    ...
    cout << "Total recursion steps: " << count << endl;
    return 0;
}
